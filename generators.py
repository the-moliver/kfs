from keras.engine.training import make_batches, standardize_input_data, standardize_sample_weights
import numpy as np


def make_batches_overlap(size, batch_size, overlap, filt_length):
...
def time_delay_generator_jitter(x, y, delays, batch_size, weights=None, shuffle=True, conv3d=False, jitter=True, jitter_axes=[3,4], max_jitter=1):
    ...
    while 1:
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batches):
            ...
            x_batch = standardize_input_data(x_batch, ['x_batch'])
            if y is None:
                yield x_batch
            else:
                y_batch = standardize_input_data(y[batch_ids[0], :], ['y_batch'])
...
def time_delay_generator_AE(x, delays, batch_size, shuffle=True, conv3d=False):
    ...
    while 1:
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batches):
            ...
            batch_ids = [np.maximum(0, batch_ids - d) for d in range(delays)]
            x_batch = standardize_input_data(x[batch_ids, :].transpose(tlist), ['x_batch'])
            y_batch = standardize_input_data(np.copy(x_batch[0]).reshape((x_batch[0].shape[0], -1)), ['y_batch'])
...
def time_delay_generator_conv(x, filt_length, frames_per_TR, TRs_in_model, y=None, weights=None):
    ...
    batches = make_batches_overlap(x_size_expand, batch_size, frames_per_TR*(TRs_in_model-1)+filt_length-1, filt_length)
    print(batches)
    index_array = np.minimum(x.shape[0]-1, np.arange(0, x_size_expand))
    while 1:
        for batch_index, (batch_start, batch_end) in enumerate(batches):
            batch_ids = index_array[batch_start:batch_end]
            x_batch = standardize_input_data(x[batch_ids, :][None, :], ['x_batch'])
            yield x_batch
