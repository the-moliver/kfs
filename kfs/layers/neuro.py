...
class RustSTC(Layer):
    ...
    def build(self, input_shape):
        ...
        self.W_quad_ex = self.add_weight(shape=(input_dim, self.quadratic_filters_ex),
                                         name='W_quad_ex',
                                         initializer=self.W_quad_ex_initializer,
                                         regularizer=self.W_quad_ex_regularizer,
                                         constraint=self.W_quad_ex_constraint)

        self.W_quad_sup = self.add_weight(shape=(input_dim, self.quadratic_filters_sup),
                                          name='W_quad_sup',
                                          initializer=self.W_quad_sup_initializer,
                                          regularizer=self.W_quad_sup_regularizer,
                                          constraint=self.W_quad_sup_constraint)

        self.W_lin = self.add_weight(shape=(input_dim, 1),
                                     name='W_lin',
                                     initializer=self.W_lin_initializer,
                                     regularizer=self.W_lin_regularizer,
                                     constraint=self.W_lin_constraint)
...
class NakaRushton(Layer):
    ...
    def build(self, input_shape):
        ...
        self.beta_delta = self.add_weight(shape=(2, 1),
                                          name='beta_delta',
                                          initializer=self.beta_delta_initializer,
                                          regularizer=self.beta_delta_regularizer,
                                          constraint=self.beta_delta_constraint)

        self.gamma_eta = self.add_weight(shape=(2, 1),
                                     name='gamma_eta',
                                     initializer=self.gamma_eta_initializer,
                                     regularizer=self.gamma_eta_regularizer,
                                     constraint=self.gamma_eta_constraint)


        self.rho = self.add_weight(shape=(1,),
                                     name='rho',
...
class EminusS(Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = K.variable(np.array([[1.], [-1.]], dtype='float32'),
